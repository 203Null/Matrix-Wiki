...
### Improved MIDI Adapter

The existing MIDI port adapter is a quick solution for users who need to connect their Mystrix to a MIDI device.

In the future, we may release a more advanced dongle that can be connected to the Mystrix's USB-C port to provide MIDI in and out ports or even CV ports.

If this happens, we will provide credit to everyone who purchased the existing MIDI port adapter.
...
You can learn more about the MIDI routing system in the Matrix OS MIDI API [here](/docs/Developer/MatrixOSApplicationCppAPI/MIDI) if you want to develop your own application that uses advanced MIDI routing with the adapter.
