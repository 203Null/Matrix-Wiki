...
The `UI4pxNumber` class represents a compact numeric display component for UI systems. It renders numbers in a 4-pixel-height format with configurable color, digit count, and spacing.
...
This component does not render negative numbers.

If your UI needs negative values, render a custom minus sign in app code. See the `minusSign` implementation in `SetOffset` in <GithubLink path="Devices/Mystrix1/Applications/ForceCalibration/ForceCalibration.cpp"/>.
:::

*The source file for this class is located in <GithubLink path="OS/UI/Component/UI4pxNumber.h"/>.*
...
- Negative numbers need custom app-side rendering.
