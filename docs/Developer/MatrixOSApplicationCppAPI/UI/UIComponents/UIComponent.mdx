...
*The source file for this class is located in <GithubLink path="OS/UI/Component/UIComponent.h"/>.*
...
```
Handles a keypad-class input event at the specified position. The UI system calls this method when an `InputEvent` maps to the component. Override this method to process keypad events.

**Parameters:**
- `xy` ([`Point`](../../Types/Point/)): The position of the event.
- `keypadInfo` (`KeypadInfo*`): Information about the keypad event, including state, pressure, velocity, hold status, and timing.
