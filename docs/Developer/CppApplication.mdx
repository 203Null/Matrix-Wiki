...
The examples below use the current `MatrixOS::Input`, `InputEvent`, and `KeypadInfo` APIs.
:::

## Native Application System Overview

In Matrix OS, each native C++ application runs on the device in its own OS thread. Native apps can use Matrix OS APIs, device-layer APIs, FreeRTOS APIs, and third-party C/C++ libraries. This gives native apps the best performance and integration, but it also makes them less portable than Python apps.

The application structure is similar to Arduino sketches: `Setup()` initializes the app, `Loop()` runs repeatedly while the app is active, `End()` cleans up when the app exits, and `Exit()` can be called when the app needs to close itself.

## Available API

- Matrix OS C++ API - Matrix OS-provided APIs. These are the preferred APIs for portability and memory safety.
- Device Layer API - Device-level APIs for hardware features that are not exposed through the Matrix OS C++ API.
- FreeRTOS API - Matrix OS runs on FreeRTOS, so native apps can use FreeRTOS APIs for threads and cross-thread communication.
- Third-party C/C++ libraries - Native apps can include libraries inside the application folder when the library can be built with the Matrix OS toolchain.

:::warning[Beta API & Build system]
The API and build system are still evolving during the beta. Application code may need updates as the platform changes.
:::

## Example App

This page walks through a native Matrix OS application structure. If you are updating older 3.x code, read [Migration From 3.x to 4.x](./MatrixOSApplicationCppAPI/MigrationFrom3To4) before converting input handling.

Use the Matrix OS source tree as the source of truth for current native app examples.
...
This includes the `Application.h` file, which defines the base class for all applications.
...
This includes the `BrightnessControl.h` file, which is an external UI for brightness control. This shows that you can include UI elements from other applications, although shared app UI should be used carefully.
...
This is the function definition of the UIMenu(), InputEventHandler(), and MidiEventHandler() functions. They are used by the application to handle UI, input, and MIDI events. We will explain them in the source file section.
...
void ExampleAPP::Loop() {
  ...
  InputEvent inputEvent; // Variable for the latest input event to be stored at
  while (MatrixOS::Input::Get(&inputEvent)) // While there is still input event in the queue
  { InputEventHandler(&inputEvent); } // Handle it
...
// Handle the input event from the OS
void ExampleAPP::InputEventHandler(InputEvent* inputEvent) {
  ...
  Point xy;
  if (MatrixOS::Input::GetPosition(inputEvent->id, &xy)) // If XY is valid, this input maps to a coordinate
  {
    MLOGD("Example", "Input %d %d %d", xy.x, xy.y, inputEvent->keypad.state); // Print the input event to the debug log
  ...
  {
    if (inputEvent->id == InputId::FunctionKey())  // The function key is exposed through the Input API
    {
      UIMenu();                 // Open UI Menu
    }
  }
...
This includes `Example.h`, the application header, and `ui/UI.h`, the Matrix OS UI framework. The UI framework is a built-in library, not part of the core Matrix OS kernel, so native apps include it explicitly when they use UI components.
...
In this case, it is looping to check if there is any input or MIDI event in the queue and handle them until the queue is empty.
...
void ExampleAPP::InputEventHandler(InputEvent* inputEvent) {
    ...
    {
      if (inputEvent->id == InputId::FunctionKey())  // Function key input
      {
        UIMenu();                 // Open UI Menu
      }
    }
...
This is the InputEventHandler() function of the application. It is called when there is an input event. It is used to handle keypad-class input events.
...
You will get an `InputEvent` with an `InputId` and an input class. For keypad input, `inputEvent->keypad` contains the state of the key event (`KeypadState::Pressed`, `KeypadState::Hold`, `KeypadState::Aftertouch`, `KeypadState::Released`) and pressure/velocity data.

You can use `MatrixOS::Input::GetPosition(inputEvent->id, &xy)` to get the XY coordinate for coordinate-addressable inputs.

`InputId::FunctionKey()` identifies the system function input. In the case of Mystrix, it is the center key. All Matrix OS devices are required to have a function key or equivalent method of invoking it.
...
Add your application to the USER APPLICATION section of the file by including the application header. In this example, the header is `applications/Example/Example.h`.
...
You can create your application in its own repository and include it as a submodule in the Matrix OS repository. This lets you develop the application separately and include it as an external module in specific builds.

We are working on standardizing this process so applications can be added to the Matrix OS build system more easily.
...
- If you use threads, mutexes, peripheral configuration, or other hardware state, clean up or undo those changes when the application exits.
